...
def create_poll(request: Request, poll: Poll):
    try:
        poll_data = poll.dict()

        slug = slugify.slugify(poll_data['title'])

        existing_poll = request.app.state.polls.find_one({'slug': slug})
        counter = 1
        new_slug = None
        while existing_poll:
            new_slug = f"{slug}-{counter}"
            existing_poll = request.app.state.polls.find_one({'slug': new_slug})
            counter += 1
        if new_slug:
            slug = new_slug
        poll_data['slug'] = slug
        print(poll_data)
        result = request.app.state.polls.insert_one(poll_data)
        print(result)
...
def get_poll(poll_id: str, request: Request):
    try:
        ...
        poll_data = request.app.state.polls.find_one({'_id': poll_id})
        if poll_data:
...
def get_poll(poll_slug: str, request: Request):
    try:
        poll_data = request.app.state.polls.find_one({'slug': poll_slug})
        if poll_data:
            poll_data['_id'] = str(poll_data['_id'])
